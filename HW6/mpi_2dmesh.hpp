...
public:
  ...
  int nleft = MPI_PROC_NULL, nrght = MPI_PROC_NULL, ntop = MPI_PROC_NULL,
      nbot = MPI_PROC_NULL;
  ...
  // overall mesh
  Tile2D(int tx, int ty, int xsize, int ysize, int rank, int nx, int ny, int nX,
         int nY) {
    xloc =
    ...
    nleft = (nx > 0) ? rank - 1 : MPI_PROC_NULL;
  ...
  void print(int row, int col) {
    printf(" Tile at [%d, %d], \tx/yloc: (%d, %d),\tbase grid size "
           "[%d,%d],\trank=%d,\tgxmin/gxmax/gymin/"
           "gymax=[%d,%d,%d,%d],\tinputBuffer.size()=%zu, "
           "outputBuffer.size()=%zu \n",
           row, col, xloc, yloc, width, height, tileRank, ghost_xmin,
           ghost_xmax, ghost_ymin, ghost_ymax, inputBuffer.size(),
           outputBuffer.size());
  }
