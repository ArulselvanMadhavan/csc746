...
int main(int argc, char **argv) {
  ...
  std::cout << std::fixed << std::setprecision(2); // Sets precision

  std::vector<int> test_sizes{64, 128, 256, 512, 1024, 2048};
  std::vector<int> block_sizes{2, 8, 16, 32, 64};
  ...
  for (int n : test_sizes) {
    ...
    for (int b : block_sizes) {
      ...
      double *A = buf.data() + 0;
      double *B = A + n * n;     // Start B at the end of A
      double *C = B + n * n;     // Start C at tne end of B
      double *Acopy = C + n * n; // Start Acopy at end of C and so on
