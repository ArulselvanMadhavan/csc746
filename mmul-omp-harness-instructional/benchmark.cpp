...
//
// On the command line you may optionally set the problem size (-N
// problemSizeInt), as well as optionally set the block size (-B blockSizeInt).
//
// If you specify nothing on the command line, the benchmark will iterate
// through a prescribed set of problem sizes, which are defined in the code
// below.
//
// For the blocked version, if you don't specify a block size on the command
// line, then the benchmark will iterate of a prescribed set of block sizes,
// which are defined in the code below.
...
#pragma omp single
    {
      ...
      // std::cout << "Hello world, I'm thread " << thread_id << " out of "
      //           << nthreads << " total threads. " << std::endl;
    }
