...
int main(int argc, char **argv) {
  ...
  // preallocate memory buffers for all problems: assume the last number in
  // test_sizes is the largest

  // allocate memory for 2 NxN matrices and 4 Nx1 vectors
  
  unsigned long long max_size = test_sizes[n_problems - 1];
  ...
  for (int n : test_sizes) {
    printf("Working on problem size N=%d\tThreads=%s\tSchedule=%s\n", n, std::getenv("OMP_NUM_THREADS"),std::getenv("OMP_SCHEDULE"));
    ...
    std::chrono::time_point<std::chrono::high_resolution_clock> ref_start_time =
        std::chrono::high_resolution_clock::now();

    reference_dgemv(n, Acopy, Xcopy, Ycopy);
    std::chrono::time_point<std::chrono::high_resolution_clock> ref_end_time =
        std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> ref_elapsed = ref_end_time - ref_start_time;
    std::cout << " Ref Elapsed time is : " << ref_elapsed.count() << " "
              << std::endl;
